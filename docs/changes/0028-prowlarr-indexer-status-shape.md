...
- **Decision:** Leave `indexerId` nullish and keep the zero fallback.
  ...
  - **Residual cost, recorded deliberately:** with `id` gone, every declared member of this schema is nullish, so the array now admits any JSON array of objects, and a future Prowlarr that renamed the identity member would yield rows reporting `indexerId: 0` rather than refusing. That is the price of the zero fallback, not of this change — it was already the behavior for a row that omitted `indexerId` — and the acquisition adapter's read of the same route still requires the member, so a renamed identity would not pass unnoticed across the whole server. Anyone revisiting it should start from `indexerRef`'s zero, which is the decision that makes the tolerance reachable.
...
- Repairing any fixture other than the Prowlarr indexer-status one, or adding the capture procedure — that is [0021](./0021-live-verified-fixtures.md). 0021 requires every adapter change its sweep turns up to become one of its own prerequisites, so this change records that edge in `docs/index.yml` and `docs/index.md`, where anyone scheduling work reads it. Restating it in 0021's own header is 0021's edit to make.
