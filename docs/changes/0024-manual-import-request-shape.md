...
- **Decision:** Build the element without the scan's episodes and season where a correction moves the file to another media and names no episodes.
  - **Why:** Found once reprocessing worked at all, and it is this change's own requirement that "what is validated remains what a caller selected". A correction moves the media alone; the episodes came from the reference, and an episode belongs to the series it is an episode of whatever the mapping around it is renamed to. Sending both builds an element that reads as coherent and is not, and Sonarr 4.0.19.2979 answers that one with `200` and no rejections at all — so a caller asking to move a file to series 1 would have had series 93's episode attached to it, upstream-approved, with nothing in this server's guard path able to refuse it. Omitting the two fields is what lets the application decide the incomplete mapping for itself: the same file and the same corrected series then come back with a **permanent** rejection, which the existing rejection guard already stops the import on, and the episodes that answer names belong to the corrected series rather than to the one the file was moved off.
  - **Alternatives considered:** Refusing the correction in the compilation, rejected because the compilation sees the caller's five fields and not the row they are decided against, so it cannot tell a media correction that moves the file from one that restates where it already is — and because refusing there would substitute this server's opinion for an answer the application is willing to give. Carrying the season while dropping the episodes, rejected because a season counts within one series and the instance reads it as a reason not to derive a mapping at all.
...
- [x] Restore manual import end to end
  ...
  - [x] Keep a correction that moves the file to another media from carrying the scan's episodes and season into the element, on the reprocess and on the command alike
