...
**Prerequisite: the App ID repository variable.** `RELEASE_PLEASE_PRIVATE_KEY` is already a repository secret. The App's numeric ID is not secret and is not stored yet; a maintainer must add it as the repository variable `RELEASE_PLEASE_APP_ID` (`gh variable set RELEASE_PLEASE_APP_ID --body <id>`, or Settings → Secrets and variables → Actions → Variables). Until it exists, the release job fails at the token step, which is also why the task verifying that a release pull request runs the quality-gate workflow remains open.

**Prerequisite: the first publication is manual.** npm can only attach a trusted publisher to a package that already exists, so the trusted publisher cannot be configured before `mcp-arr` has been published once. A maintainer performs the first publication by hand from a clean checkout of the release tag (`npm ci && npm run check && npm publish`, authenticated interactively), then configures the trusted publisher on npmjs.com against this repository and `.github/workflows/release.yml`. From the second release onward the workflow publishes over OIDC with no stored credential.
...
- [x] **Resolved — the first publication is performed manually.** npm requires the package to exist before a trusted publisher can be configured against it, so a maintainer publishes the first version by hand and configures the trusted publisher afterwards. Automation publishes every release from the second onward; see [Releasing](#releasing).
