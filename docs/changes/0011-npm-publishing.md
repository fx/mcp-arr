...
**Prerequisite: the first publication is manual, and it comes first.** npm can only attach a trusted publisher to a package that already exists, so the trusted publisher cannot be configured before `mcp-arr` has been published once. That bootstrap must happen **before the first release pull request is merged**, or the publish job would run against a package that has no trusted publisher yet and fail after the tag had already been cut. The order is: land the package metadata, publish the current version by hand from a clean checkout of the default branch (`npm ci && npm run check && npm publish`, authenticated interactively), configure the trusted publisher on npmjs.com against this repository and `.github/workflows/release.yml`, and only then merge a release pull request. Every release from that point publishes over OIDC with no stored credential.
