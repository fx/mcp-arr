...
- **Decision:** Report a completed command whose result is present but indefinite as unknown state, not as a terminal one.
  ...
  - **Evidence, stated plainly:** this mapping is **inferred, not observed.** Every other claim in this document is backed by a live probe; this one is not. Producing an orphaned command needs an application restarted with a command in flight, which the probes could not arrange, so the mapping rests on what the status means and on the fact that this project's own activity vocabulary already lists it (`src/adapters/activity/model.ts`). No evidence was found that an orphaned command resumes. If one is ever observed resuming, this mapping is the thing to revisit.
