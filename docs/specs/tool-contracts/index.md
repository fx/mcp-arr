...
- A published input schema MUST NOT declare a maximum length for a string value. The bound MUST remain in validation and MUST be stated, naming the property it bounds and the length it allows, in documentation generated into the published root's own description — every published root, including one whose input is a plain object and therefore carries no variant documentation for it to join. A host that compiles a published schema into a constrained-decoding grammar expands a length bound once per admissible character, and a schema it cannot compile costs the caller the whole tool rather than the bound. A minimum length, a pattern, and item counts MUST still be published: a pattern constrains the admissible alphabet far more usefully than a length, and an item count is the bound a caller acts on.
...
- A projection MUST be bounded in the number of paths it may name and in the length of one path, and both bounds MUST be at least what the tool's own generated path inventory publishes — the largest number of leaf paths any single payload names, and the longest path in any inventory — so no bound can refuse a projection naming exactly what the tool advertises. The path-length bound is a string length and MUST therefore not be published; the count of paths is an item count and MUST be.
- A projection MUST NOT change the record counts a result reports. Those describe what the query matched, which is what a caller pages on, not what the projection kept.
