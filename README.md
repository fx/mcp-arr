...
Sanitizing drops any key the screens name as secret-bearing or identifying, and replaces any value they refuse — a URL, an address, a home or host path — with a neutral stand-in of the same kind, so the shape survives and the text does not. If a capture is refused, neutralize the value the message names. Never weaken a screen to accommodate what an instance sent.
